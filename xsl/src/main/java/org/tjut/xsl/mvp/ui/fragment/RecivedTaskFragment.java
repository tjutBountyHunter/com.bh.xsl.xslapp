...
import org.tjut.xsl.di.component.DaggerRecivedTaskComponent;
import org.tjut.xsl.mvp.contract.RecivedTaskContract;
import org.tjut.xsl.mvp.model.entity.Task;
import org.tjut.xsl.mvp.presenter.RecivedTaskPresenter;
 ...
 * ================================================
 */
public class RecivedTaskFragment extends BaseFragment<RecivedTaskPresenter> implements RecivedTaskContract.View {


    ReciveAdapter mReciveAdapter;
    private QMUIEmptyView mEmptyView;
    @BindView(R.id.rrecycler_sent)
    RecyclerView mRecyclerView;
    private String TAG = this.getClass().getSimpleName();

    public static RecivedTaskFragment newInstance() {
        RecivedTaskFragment fragment = new RecivedTaskFragment();
        return fragment;
    }

    @Override
    public void setupFragmentComponent(@NonNull AppComponent appComponent) {
        DaggerRecivedTaskComponent //如找不到该类,请编译一下项目
                .builder()
    ...
    public View initView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        return inflater.inflate(R.layout.fragment_recived_task, container, false);
    }

    @Override
    public void initData(@Nullable Bundle savedInstanceState) {
        mReciveAdapter = new ReciveAdapter(R.layout.item_sent_task);
        mRecyclerView.setLayoutManager(new LinearLayoutManager(getContext(), LinearLayoutManager.VERTICAL, false));
        mRecyclerView.setAdapter(mReciveAdapter);
        mReciveAdapter.setOnItemClickListener((adapter, view, position) -> {
    ...
    public void showTask(List<Task> tasks) {
        mReciveAdapter.replaceData(tasks);
    }

    @Override
    public void showEmptyView() {
        if (mReciveAdapter.getEmptyView() == null) {
            mEmptyView.setTitleText("暂时没有已发任务，快去发布吧");
            mReciveAdapter.setEmptyView(mEmptyView);
        }
    }

    class ReciveAdapter extends BaseQuickAdapter<Task, BaseViewHolder> {

        public ReciveAdapter(int layoutResId, @Nullable List<Task> data) {
            super(layoutResId, data);
        }

        public ReciveAdapter(@Nullable List<Task> data) {
            super(data);
        }

        public ReciveAdapter(int layoutResId) {
            super(layoutResId);
        }
